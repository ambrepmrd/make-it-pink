function applyRoseTint(element) {
  ...
  if (currentColorBg && currentColorBg !== 'rgba(0, 0, 0, 0)' && !isBlackOrWhite(currentColorBg)) {
    if (currentColorBg.includes("linear-gradient")) {
      ...
      if (gradientMatch) {
        ...
        element.style.background = "linear-gradient(" + newGradientColors.join(', ') + ")";
      }
    } else {
  ...
  pseudoelements.forEach(function(pseudoelement) {
    ...
    if (!isBlackOrWhite(currentPseudoelementBgColor)) {
      let rgbValuesPseudoelementBg = extractRGBA(currentPseudoelementBgColor);
      if (rgbValuesPseudoelementBg) {
        let hslValuesPseudoelementBg = rgbToHsl(...rgbValuesPseudoelementBg);
        hslValuesPseudoelementBg[0] = 330 / 360;
        let modifiedRgbPseudoelementBg = hslToRgb(...hslValuesPseudoelementBg);
...
function isBlackOrWhite(color) {
  ...
  if (rgb) {
    ...
    return (rgb[0] < 50 && rgb[1] < 50 && rgb[2] < 50) || (rgb[0] > 200 && rgb[1] > 200 && rgb[2] > 200);
  }
  return false;
}
